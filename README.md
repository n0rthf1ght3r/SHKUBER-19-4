# Домашнее задание к занятию «Хранение в K8s» - Крюков Егор
...
**Задача:** Создать Deployment с двумя контейнерами (busybox пишет, multitool читает), используя общий том `emptyDir`.

**Манифест:** [containers-data-exchange.yaml](./containers-data-exchange.yaml)

### Результаты проверки
На скриншоте ниже представлено описание пода (видно два контейнера и общий Volume), а также вывод команды `tail` из контейнера `multitool`, подтверждающий чтение данных.
![Скриншот](./1.png)
...
**Задача:** Использовать PersistentVolume и PersistentVolumeClaim для сохранения данных на диске ноды.

**Манифест:** [pv-pvc.yaml](./pv-pvc.yaml)

### Результаты проверки

**1. Проверка записи и чтения:**
Контейнер `multitool` успешно читает файл, который пишется в примонтированную директорию.
![Скриншот](./2.png)

**2. Поведение при удалении Deployment и PVC:**
После удаления PVC статус PersistentVolume изменился на `Released`.
![Скриншот](./3.png)

> **Пояснение:** Статус `Released` означает, что PVC был удален, и ресурс свободен от привязки, но данные еще не очищены (так как политика ReclaimPolicy установлена в `Retain`).

**3. Поведение при удалении PV:**
После удаления самого объекта PV, файл на физическом диске ноды остался нетронутым.
![Скриншот](./4.png)

> **Пояснение:** Файл сохранился, потому что Kubernetes управляет только своими объектами. При использовании драйвера `hostPath` удаление объекта PV не приводит к удалению файлов в файловой системе операционной системы хоста.

---

## Задание 3. StorageClass

**Задача:** Создать StorageClass и запустить приложение с его использованием.

**Манифест:** [sc.yaml](./sc.yaml)

### Результаты проверки
StorageClass успешно создан, PV и PVC связались, приложение пишет и читает данные.
![Скриншот](./5.png)
